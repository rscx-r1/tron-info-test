...
class TronRequestReadSchema(BaseModel):
    ...
    bandwith: int = Field(description="Количество использованной пропускной способности.")
...
class TronWalletSchema(BaseModel):
    ...
    def validate_address(cls, value: str) -> str:
        if not value.startswith("T"):
            raise exceptions.InvalidTRONAddressException("Адрес кошелька должен начинаться с символа 'T'.")
        return value
...
class TronRequestCreateSchema(TronWalletSchema):
    ...
    bandwidth: int = Field(description="Количество использованной пропускной способности.")
    energy: int = Field(description="Количество использованной энергии.")
    trx_balance: Decimal = Field(description="Количество TRX на кошельке.")
